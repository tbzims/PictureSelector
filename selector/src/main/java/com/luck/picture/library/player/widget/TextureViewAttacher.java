...
package com.luck.picture.library.player.widget;
...
import com.luck.picture.library.photoview.OnMatrixChangedListener;
import com.luck.picture.library.photoview.OnScaleChangedListener;
import com.luck.picture.library.photoview.OnSingleFlingListener;
import com.luck.picture.library.photoview.OnViewDragListener;
import com.luck.picture.library.photoview.OnViewTapListener;

/**
 * @author：luck
 * @date：2023/1/4 4:55 下午
 * @describe： The component of {@link TextureView} which does the work allowing for zooming, scaling, panning, etc.
 * It is made public in case you need to subclass something other than AppCompatImageView and still
 * gain the functionality that {@link TextureView} offers
 */
public class TextureViewAttacher implements View.OnTouchListener,
        ...
        mGestureDetector.setOnDoubleTapListener(new GestureDetector.OnDoubleTapListener() {
            ...
            public boolean onSingleTapConfirmed(MotionEvent e) {
                ...
                if (mViewTapListener != null) {
                    mViewTapListener.onViewTap(mTextureView, x, y);
                }
                if (displayRect != null) {
                    ...
                    if (displayRect.contains(x, y)) {
                        ...
                        float yResult = (y - displayRect.top)
                                / displayRect.height();
                        if (mTextureTapListener != null) {
                            mTextureTapListener.onTextureTap(mTextureView, xResult, yResult);
                        }
                        return true;
                    } else {
                        if (mOutsideTextureTapListener != null) {
                            mOutsideTextureTapListener.onOutsideTextureTap(mTextureView);
                        }
                    }
                }
                return false;
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        ...
        if (mZoomEnabled && TextureViewUtils.INSTANCE.hasBitmap((TextureView) v)) {
            switch (ev.getAction()) {
    ...
    public void setMinimumScale(float minimumScale) {
        TextureViewUtils.INSTANCE.checkZoomLevels(minimumScale, mMidScale, mMaxScale);
        mMinScale = minimumScale;
    }

    public void setMediumScale(float mediumScale) {
        TextureViewUtils.INSTANCE.checkZoomLevels(mMinScale, mediumScale, mMaxScale);
        mMidScale = mediumScale;
    }

    public void setMaximumScale(float maximumScale) {
        TextureViewUtils.INSTANCE.checkZoomLevels(mMinScale, mMidScale, maximumScale);
    ...
    public void setScale(float scale, boolean animate) {
        setScale(scale, (mTextureView.getRight()) >> 1, (mTextureView.getBottom()) >> 1, animate);
    }
    ...
    private RectF getDisplayRect(Matrix matrix) {
        Bitmap d = mTextureView.getBitmap();
        if (d != null) {
            mDisplayRect.set(0, 0, d.getWidth(),
                    d.getHeight());
    ...
    private class AnimatedZoomRunnable implements Runnable {
        ...
        public void run() {
            ...
            onTextureViewGestureListener.onScale(deltaScale, mFocalX, mFocalY);
